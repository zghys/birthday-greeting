...
function StarsBg() {
  const stars = Array.from({ length: 80 }, (_, i) => ({
    ...
    delay: (Math.random() * 4).toFixed(1) + 's', dur: (Math.random() * 3 + 2).toFixed(1) + 's',
  }));
  return (
...
function FallingHearts({ active }) {
  ...
  const items = Array.from({ length: 30 }, (_, i) => ({
    ...
    delay: (Math.random() * 3).toFixed(1) + 's', dur: (Math.random() * 3 + 3).toFixed(1) + 's',
    size: (Math.random() * 12 + 8).toFixed(0) + 'px', type: Math.random() > 0.5 ? 'heart' : 'petal',
  }));
  return (
    <div className="fixed inset-0 pointer-events-none z-50 overflow-hidden">
      {items.map(item => (
        <div key={item.id} className="absolute animate-fall"
          style={{ left: item.left, top: '-30px', fontSize: item.size, animationDelay: item.delay, animationDuration: item.dur, opacity: 0.7 }}>
          {item.type === 'heart' ? '❤️' : '🌸'}
        </div>
      ))}
    </div>
...
function Confetti({ active }) {
  ...
  const pieces = Array.from({ length: 60 }, (_, i) => ({
    id: i, left: Math.random() * 100 + '%',
    delay: (Math.random() * 2).toFixed(1) + 's', dur: (Math.random() * 2 + 2).toFixed(1) + 's',
    color: colors[i % colors.length], size: (Math.random() * 8 + 4).toFixed(0) + 'px', rotate: Math.random() * 360,
  }));
  return (
    <div className="fixed inset-0 pointer-events-none z-50 overflow-hidden">
      {pieces.map(p => (
        <div key={p.id} className="absolute animate-confetti"
          style={{ left: p.left, top: '-20px', width: p.size, height: p.size, backgroundColor: p.color, borderRadius: Math.random() > 0.5 ? '50%' : '2px', transform: `rotate(${p.rotate}deg)`, animationDelay: p.delay, animationDuration: p.dur, opacity: 0.9 }} />
      ))}
    </div>
  );
}
...
function Home() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (!name) { setInputError('请输入你的名字'); return; }
    setInputError('');
    setSubmitting(true);
  ...
  const closeBlessing = useCallback(() => {
    ...
    setTimeout(() => loveWallRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100);
  }, []);
  ...
  const message = settings.blessing_message || '祝你生日快乐！';
    ...
    <div className="relative min-h-screen bg-birthday-bg overflow-x-hidden">
      ...
      <section className="relative z-10 min-h-screen flex flex-col items-center justify-center px-4 py-12">
        ...
        <div className="text-center max-w-2xl mx-auto">
          <div className="animate-float mb-6">
            <h1 className="text-6xl md:text-8xl font-display font-bold bg-gradient-to-r from-birthday-gold via-birthday-pink to-birthday-purple bg-clip-text text-transparent mb-2">🎂 生日快乐</h1>
          </div>
          ...
          <div className="max-w-md mx-auto">
            ...
            <div className="flex gap-3">
              <input type="text" value={visitorName} onChange={e => { setVisitorName(e.target.value); setInputError(''); }} onKeyDown={handleKeyDown}
                placeholder="你的名字" className="flex-1 px-5 py-3.5 bg-birthday-surface/60 border border-birthday-purple/30 rounded-xl text-birthday-text placeholder-birthday-muted/30 focus:border-birthday-gold/50 focus:ring-2 focus:ring-birthday-gold/20 transition-all backdrop-blur-sm text-lg" maxLength={20} autoFocus />
            ...
            {inputError && <p className="text-birthday-pink/80 text-sm mt-2 text-left">{inputError}</p>}
          </div>
        </div>
      ...
      {showBlessing && (
        <div className="fixed inset-0 z-40 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={closeBlessing}>
          <div className="bg-birthday-surface/90 backdrop-blur-xl rounded-3xl p-10 max-w-md w-full border border-birthday-gold/30 shadow-2xl animate-fade-in-up text-center relative overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-birthday-text text-xl mb-1"><span className="text-birthday-pink font-bold">{blessedName}</span></p>
            <div className="h-px bg-gradient-to-r from-transparent via-birthday-gold/30 to-transparent my-6" />
            <p className="text-birthday-text/80 text-lg mb-8 leading-relaxed">{message}</p>
            <button onClick={closeBlessing}
      ...
      <section ref={loveWallRef} className={`relative z-10 py-20 px-4 transition-all duration-700 ${showLoveWall ? 'opacity-100' : 'opacity-0'}`}>
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-display font-bold text-birthday-text mb-2">❤️ 爱心墙</h2>
          ...
          <div className="max-w-6xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {photos.map((photo, index) => (
                <div key={photo.id} className="group relative rounded-2xl overflow-hidden bg-birthday-surface/40 border border-birthday-pink/10 hover:border-birthday-pink/40 transition-all duration-500 hover:shadow-[0_0_30px_rgba(236,72,153,0.2)] hover:-translate-y-1">
                  <div className="aspect-[4/3] overflow-hidden">
                    <img src={photo.photo_url} alt={photo.caption || '照片'} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" loading="lazy"
                      onError={(e) => { e.target.style.display = 'none'; e.target.parentElement.classList.add('flex', 'items-center', 'justify-center', 'bg-gradient-to-br', 'from-birthday-pink/20', 'to-birthday-purple/20'); e.target.parentElement.innerHTML = '<span class="text-4xl">📷</span>'; }} />
                  </div>
                  {photo.caption && (
                  ...
                  <div className="absolute top-3 right-3 text-birthday-pink/0 group-hover:text-birthday-pink/60 transition-all duration-300 text-lg">❤️</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="text-center mt-16 pb-8">
          <p className="text-birthday-muted/30 text-sm">愿每一份祝福都化作最美的回忆</p>
          <Link to="/admin/login" className="inline-block mt-2 text-birthday-muted/15 text-xs hover:text-birthday-muted/40 transition-colors">管理</Link>
        </div>
      </section>
    </div>
  );
